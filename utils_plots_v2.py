...
def getTranslatedNodes(features, coefs, num_to_node, g, save_location = None):
    ...
    """
    Generates subgraph with selected features

    Parameters:
    -----------
    features : array-like
               list of feature indicies (with respect to feature order in covariates)

    coefs : array-like
            list of coefficients for features

    num_to_node : dict
                  dictionary to convert feature index to gene name
...
def getGridsearchPlot(gridsearch_results, alpha_list, threshold_list, save_location = None):
    ...
    gridsearch_results : Pandas dataframe,
                         With columns ['alpha', 'threshold', 'num_features', 'Train MSE', 'Test MSE']
